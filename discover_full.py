...
extracts sub-category links from the left sidebar (zg_browseRoot),
...
def extract_subcats(page):
    """Extract sub-category links from the left sidebar (zg_browseRoot)."""
    links = page.evaluate("""() => {
        const out = [];
        // Amazon's left sidebar uses zg_browseRoot
        const root = document.querySelector("#zg_browseRoot");
        if (!root) return out;
        root.querySelectorAll("a[href*='/gp/bestsellers/']").forEach(a => {
            const href = a.href;
...
def crawl_category_page(slug, parent_node=None, depth=0, max_depth=3):
