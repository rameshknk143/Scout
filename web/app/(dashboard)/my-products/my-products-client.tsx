...
export default function MyProductsClient() {
  ...
  const columns = [
    ...
    },
    {
      key: "supplier_cost",
      header: "Unit Cost",
      render: (p: MyProduct) => <span className="font-medium text-zinc-800">₹{p.supplier_cost.toLocaleString("en-IN")}</span>,
    },
    {
      key: "shipping_fee",
      header: "Shipping",
      render: (p: MyProduct) => <span className="font-medium text-zinc-500">₹{p.shipping_fee.toLocaleString("en-IN")}</span>,
    },
    {
      key: "total_cost",
      header: "Total Cost",
    ...
    },
    {
      ...
      render: (p: MyProduct) => (
        <span className="font-bold text-emerald-800 bg-emerald-50 border border-emerald-100 rounded px-1.5 py-0.5 text-[10px]">
          {p.target_margin}%
        </span>
      ),
    },
    {
      ...
      render: (p: MyProduct) => (
        <button
          ...
          className="text-[10px] font-bold text-red-700 bg-red-50 hover:bg-red-100 px-2 py-1 rounded transition-colors border border-red-100/50 cursor-pointer"
        >
          Remove
        </button>
      ),
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Sourcing Add Panel */}
        <div className="lg:col-span-4 bg-white border border-zinc-200/80 rounded-xl p-5 shadow-sm space-y-4 h-fit">
          <div className="text-sm font-semibold text-zinc-950">Add Catalog Listing</div>
          <form onSubmit={handleAddProduct} className="space-y-3.5">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full h-20 border border-zinc-200 rounded-lg p-2.5 text-xs outline-none focus:border-zinc-400 text-zinc-950 resize-none leading-relaxed"
              />
            </div>

            {formError && <div className="text-xs text-red-700 font-semibold">{formError}</div>}
        ...
        <div className="lg:col-span-8 bg-white border border-zinc-200/80 rounded-xl p-5 shadow-sm space-y-4">
          <div className="text-sm font-semibold text-zinc-950">Active Catalog Trackings</div>
          {isPending ? (
            <div className="flex flex-col items-center justify-center py-16 space-y-3">
              <div className="w-6 h-6 border-2 border-zinc-950 border-t-transparent rounded-full animate-spin"></div>
              <div className="text-xs text-zinc-400 font-semibold">Updating catalog list...</div>
            </div>
          ) : (
            <Table
              columns={columns}
