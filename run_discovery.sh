#!/bin/bash
# Sub-category discovery crawl: extracts every browse-node ID from Amazon's
# bestseller tree. Runs on the VM (has playwright + chromium). Outputs a JSON
# tree the collector can consume.
...
cat > /home/ubuntu/discover_nodes.py <<"PYEOF"
#!/usr/bin/env python3
"""
...
def extract_links(page):
    ...
    # Escape forward slashes in CSS selector
    links = page.evaluate("""() => {
        const out = [];
...
PYEOF
python3 /home/ubuntu/discover_nodes.py
'